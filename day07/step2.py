...
img1 = cv.imread(get_sample('book_cap.png', repo='opencv'), cv.IMREAD_GRAYSCALE)
img2 = cv.imread(get_sample('book_pic.png', repo='opencv'), cv.IMREAD_GRAYSCALE)
...
if len(good_matches) >= MIN_MATCH_COUNT:
    ...
    if M is not None:
        ...
        draw_matches(img1, kp1, img2, kp2, inlier_matches, 
                     title=f"Inliers ({len(inlier_matches)})")
