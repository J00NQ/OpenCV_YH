...
def apply_mosaic(frame, faces, block_size=15):
    ...
    for (x, y, w, h) in faces:
        ...
        # 축소 (다운샘플링)
        small = ...  # 수정: cv2.resize(face_roi, (block_size, block_size))
        
        # 다시 확대 (업샘플링) - INTER_NEAREST: 블록화 효과
        mosaic = cv2.resize(small, (w, h), interpolation=...)
...
'''
모자이크 강도 실험:
block_size를 바꿔가며 효과를 비교하세요.
'''
# 약한 모자이크
frame = apply_mosaic(frame, faces, block_size=5)   # 개인 식별 가능
# 중간 모자이크
frame = apply_mosaic(frame, faces, block_size=15)  # 적절한 수준
# 강한 모자이크
frame = apply_mosaic(frame, faces, block_size=40)  # 거의 안 보임
