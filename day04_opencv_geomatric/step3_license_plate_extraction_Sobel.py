...
def find_license_plate(img):
    ...
    cv.imshow('Debug 1: Sobel X', sobelx)

    # # 가로선 강조 (번호판은 가로 직사각형)
    ret, thresh = cv.threshold(sobelx, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU)
    cv.imshow('Debug 2: Threshold', thresh)

    kernel = cv.getStructuringElement(cv.MORPH_RECT, (17, 3))
    morphed = cv.morphologyEx(thresh, cv.MORPH_CLOSE, kernel)
    cv.imshow('Debug 3: After Morphology', morphed)
