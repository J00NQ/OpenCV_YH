...
def find_license_plate(img):
    ...
    cv.imshow('Debug 1: Canny Edges', edges)

    # # 가로선 강조 (번호판은 가로 직사각형)
    kernel = cv.getStructuringElement(cv.MORPH_RECT, (20, 5))
    edges = cv.morphologyEx(edges, cv.MORPH_CLOSE, kernel)
    cv.imshow('Debug 2: After Morphology', edges)
...
img = cv.imread('car_front.jpg')
# img = cv.imread(sampledownload.get_sample('car_plate.jpg', 'insightbook'))
